...
(function ($, window, document, undefined) {
  ...
  function debug (message, lc) {
    if (!lc.options.debug) return
    if (typeof message === 'string') console.log(message)
    else if (typeof message === 'object') console.log(JSON.stringify(message))
  }
    ...
    switch (lc.options.type) {
      case 'table':
        ...
        for (var i = 0; i < listLength; i++) {
          ...
          if (product.Stock > 0) addToCart(product, lc)
          const tr = $('<tr>')
          tbody.append(tr)
  ...
  function addEventListeners (lc) {
    ...
    $(lc.element).on(
      ...
      function () {
        ...
        if ($(this).hasClass('plus')) cartQuantity++
        else cartQuantity--
  ...
  function updateCartTotal (lc) {
    ...
    for (let i = 0; i < lc.privateData.cart.length; i++) {
      ...
      if (lc.privateData.cart[i].quantity == 0) continue
  ...
  function updateQuantity (skuId, quantity, lc) {
    ...
    if (quantity == 0) stockIcon.addClass('quantityZero')
    else stockIcon.removeClass('quantityZero')
    let any = false
  ...
  ListaDeCompras.prototype = {
    ...
    updateQuantity: function (skuId, quantity) {
      ...
      if (!this.loaded) $.error('Load a list before call this method')
    ...
    set: function (option, value) {
      ...
      if (this.loaded) processList(this)
      return true
    },
    get: function (option) {
      if (!this.initialized) {
  ...
  $.fn[pluginName] = function (arg) {
    ...
    if (
      ...
      if (arg !== 'destroy') return returnValue
